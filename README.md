...
9. `npm run audit:targets`로 transcript 기반 target-expression/context/example 검증을 적용합니다. 이 단계에서 `X보다는 Y` 패턴처럼 영상의 실제 교육 포인트가 Y인 경우 X를 daily/quiz 후보에서 제외하고, 뜻은 맞지만 사용 상황이 transcript와 맞지 않는 항목이나 제대로 된 예문이 없는 항목도 daily/quiz 후보에서 제외합니다.
